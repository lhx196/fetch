export type TConfig = {
  ...
  method?: 'GET' | 'POST' | 'PUT' | 'DELETE'; // 请求类型，部分后端只能识别大写
...
export default class FetchRequest {
  ...
  request = (configs: TFetchRequset) => {
    ...
    if (JSON.stringify(defaultConfig) !== '{}') requestConfig = Object.assign({}, requestConfig, defaultConfig);
    const data = Object.assign({}, requestConfig, { body: JSON.stringify(body) });

    return Promise.race([
      fetch(url, data),
      new Promise((_, reject) => setTimeout(reject, 5000, '请求超时')), // 请求超时
    ])
      .then((response) => {
